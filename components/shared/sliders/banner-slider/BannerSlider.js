...
const BannerSlider = ({ slides, slidesByIndex }) => {
  // animation state
  const [animation, setAnimation] = useState(false);
  // key state for slides
  const [carouselRandomNumber, setCarouselRandomNumber] =
    useState();
  ...
  const [prevBtnEnabled, setPrevBtnEnabled] =
    useState(false);
  const [nextBtnEnabled, setNextBtnEnabled] =
    useState(false);
